...
// 01. CREATE MAP (지도 생성하기)
      ...
      daum.maps.event.addListener(map, 'click', function(mouseEvent) {
          searchDetailAddrFromCoords(mouseEvent.latLng, function(result, status) {
              if (status === daum.maps.services.Status.OK) {
                  ...
                  // 날씨 정보 반횐
                   var str = realTimeWeather(latitude, longitude);
                   content +=  '<div class="bAddr"><span class="title">기상정보</span>' + str + '</div>';
   ...
   // 06. 맛집 블로그 검색
     ...
     function realTimeWeather(latitude, longitude) {
       ...
       var _nx = Math.round(latitude);
       var _ny = Math.round(longitude);
//       alert(_nx + '' +_ny);
       ...
       /* 좌표 */
       var apikey = "u0moZKKe1XHpgJqqIymctrM1IMg5XNQFJVWNnSy%2BfPviVASgHDITF7K%2F8Rg68M1l9Xw9KIDB2lzG4rw1jxIsZA%3D%3D"   
         var ForecastGribURL = "http://newsky2.kma.go.kr/service/SecndSrtpdFrcstInfoService2/ForecastGrib?"
       ForecastGribURL += "&ServiceKey="+apikey
       ForecastGribURL += "&base_date="+today
       ForecastGribURL += "&base_time="+hours +'00'
       ForecastGribURL += "&nx="+_nx
       ForecastGribURL += "&ny="+_ny
       ForecastGribURL += "&pageNo="+ '1'
       ForecastGribURL += "&numOfRows="+ '7'
      ...
      $.ajax({
          ...
          ,success: function(msg) {
              console.log(msg);
                ...
                var arr = msg.response.body.items.item;
                for(var i = 0; i < arr.length; i++) {
                   if(arr[i].category == "T1H") {
                      temp = arr[i].obsrValue;   
                   } else if(arr[i].category == "RN1") {
                      rain = arr[i].obsrValue;
                   } else if(arr[i].category == "REH") {
                      hum = arr[i].obsrValue;
                   }
                }
                console.log(arr[0].baseDate);
